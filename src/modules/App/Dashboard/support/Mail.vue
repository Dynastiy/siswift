    ...
    <vForm @submit="onSubmit" v-slot="{ meta }" class="">
      <div class="flex flex-col gap-[10px]">
        ...
        <div>
          ...
          <vField name="priority" rules="required" class="input" as="select">
            <option selected disabled value="">--Select Priority--</option>
            <option v-for="(item, idx) in priorities" :key="idx" :value="item.value">
              {{ item.label }}
            </option>
          </vField>
          <ErrorMessage name="priority" class="text-xs text-error"></ErrorMessage>
        </div>
